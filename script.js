class Game {
   ...
   * to recalculate the scale on screen resize or device rotation
    ...
    * Màn hình trò chơi có thể mở rộng. Tôi lấy 1200x800px làm tỷ lệ ban đầu.
    * Để hiển thị trò chơi, nhiều kích thước màn hình phù hợp
    * Tôi phải so sánh kích thước sreen của người chơi với quy mô ban đầu,
    * sau đó chia tỷ lệ trò chơi bằng cách sử dụng CSS Transform để phù hợp với màn hình
    * Chức năng được gọi trong bộ điều khiển và bất cứ nơi nào tôi cần
    * để tính toán lại tỷ lệ khi thay đổi kích thước màn hình hoặc xoay thiết bị
   */
  calculateScale() {
    ...
    this.steps = this.screen / this.stickWidth; // how many steps (stick width + margin) it takes from one end to another (cần bao nhiêu bước (chiều rộng thanh + lề) từ đầu này sang đầu kia)
  }
   ...
   * 
    * Tạo bao nhiêu que tính chúng ta cần để lấp đầy màn hình
    * từ đầu đến cuối màn hình. Thuộc tính các bước được sử dụng cho điều đó
   */
  generateSticks() {
  ...
  /**
   * The greeting when the game begins (Lời chào khi trò chơi bắt đầu)
   */
  intro() {
  ...
  * Display score
  ...
  /**
   * Takes players score and generates the cheering copy (Đưa người chơi ghi điểm và tạo bản sao cổ vũ)
   * @param  {int} score
   * @return {string} grade
   */
  showGrade(score) {
  ...
  scaleScreen() {
    ...
    TweenMax.killAll(); // prevent multiple calls on resize (ngăn chặn nhiều cuộc gọi khi thay đổi kích thước)
   ...
   * 
    * Gọi hàm trên
    * Nếu trò chơi đang chạy, nó sẽ dừng lại và hiển thị điểm số
    * Nếu trò chơi dừng lại, người chơi sẽ đến menu chính
   */
  scaleScreenAndRun() {
